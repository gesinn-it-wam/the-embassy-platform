...
class CustomMain extends Component {
	...
	public function getHtml() {
		...
		$mwContent = $this->indent() . '<!-- start the content area -->' .
		...
		$cointainer_class = ($_REQUEST['title'] === "Main_Page") ? "" : "container";

		return 	$this->indent() . '<!-- start main wrapper -->' .
			$this->indent() . $idRegistry->element(
				'div',
				[ 'class' => 'wrapper ' . $cointainer_class ],
				$mwContent
			);
	}
	...
	protected function buildContentHeader() {
		...
		$ret = $this->indent() . $idRegistry->element( 'div', [ 'class' => "contentHeader" ],
				...
				$contentSub2 .
				$jumpToNav .
				$idRegistry->element( 'button', ['class' => 'search-trigger'] ) .
				$this->indent( -1 )
			);
