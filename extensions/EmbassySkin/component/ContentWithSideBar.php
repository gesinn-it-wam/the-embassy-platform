...
class ContentWithSideBar extends Component {
	...
	public function getHtml() {
		...
		$mwMain =
			$this->indent() . '<!-- start main wrapper -->' .
			$this->indent() . $idRegistry->element(
				'main',
				[ 'class' => 'content col-sm-12 col-lg-6 offset-lg-2' ],
				$mwContent
			);


		$mwASide = $this->indent() . '<!-- start aside wrapper -->' .
			$this->indent() . $idRegistry->element(
				'aside',
				[ 'class' => 'sidebar col-sm-12 col-lg-3 offset-lg-1' ]
			);

		return $mwMain . $mwASide;
	}
