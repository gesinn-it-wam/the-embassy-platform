...
class PFFormEdit extends UnlistedSpecialPage {
	...
	function printForm( $form_name, $targetName, $alt_forms = array() ) {
		...
		// >>>>> gesinn.it Issue:1371 01 patch >>>>>
		$displayTitle = '';
		if ( !empty( $targetName ) ) {
				$this->getDisplayTitle( $targetName, $displayTitle );
		}
		// <<<<< gesinn.it Issue:1371 01 patch <<<<<
		...
		if ( array_key_exists( 'formtitle', $result ) ) {
			...
			if ( empty( $targetName ) ) {
				...
				// >>>>> gesinn.it Issue:1371 02 patch >>>>>
				#$pageTitle = $result[ 'formtitle' ] . ': ' . $targetName;
				$pageTitle = $result[ 'formtitle' ] . ': ' . $displayTitle;
				// <<<<< gesinn.it Issue:1371 02 patch <<<<<
			...
			if ( empty( $targetName ) ) {
				$pageTitle = wfMessage( 'pf_formedit_createtitlenotarget', $result[ 'form' ] )->text();
				 ...
				 // >>>>> gesinn.it Issue:1371 03 patch >>>>>
				#$pageTitle = wfMessage( 'pf_formedit_edittitle', $result[ 'form' ], $targetName )->text();
				$pageTitle = wfMessage( 'pf_formedit_edittitle', $result[ 'form' ], $displayTitle )->text();
				// <<<<< gesinn.it Issue:1371 03 patch <<<<<
	...
	// >>>>> gesinn.it Issue:1371 04 patch >>>>>
	function getDisplayTitle( $pagetitle, &$displaytitle ) {
