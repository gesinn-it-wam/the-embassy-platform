...
abstract class PFFormInput {
    ...
    public function getResourceModuleScriptNames() {
		return null;
	}

	public function getResourceModuleStyleNames() {
		return null;
	}
	...
	public function addJavaScript() {
		...
		$moduleScripts = $this->getResourceModuleScriptNames();
		$moduleStyles = $this->getResourceModuleStyleNames();

		// Add JS/styles recognized by the Resource Loader. See [1] for details.
		// [1] https://www.mediawiki.org/wiki/Manual:OutputPage.php#JavaScript_and_CSS
        
		// Register combined scripts/styles for the input.
		if ( $modules !== null ) {
			$output->addModules( $modules );
		}

		// Register only scripts for the input.
		if ( $moduleScripts !== null ) {
			$output->addModuleScripts( $moduleScripts );
		}

		// Register only styles for the input.
		if ( $moduleStyles !== null ) {
			$output->addModuleStyles( $moduleStyles );
		}
