...
class PFComboBoxInput extends PFFormInput {
	...
	protected $mInitialValue;		# required by SemanticFormsSelect extension

	public function __construct( $input_number, $cur_value, $initial_value, $input_name, $disabled, $other_args ) {
		//public function __construct( $input_number, $cur_value, $input_name, $disabled, $other_args ) {
		$this->mInputNumber = $input_number;
		$this->mCurrentValue = $cur_value;
		$this->mInitialValue = $initial_value;
	...
	public static function getHTML( $cur_value, $initial_value = null, $input_name, $is_mandatory, $is_disabled, $other_args ) {
		global $wgPageFormsTabIndex, $wgPageFormsFieldNum, $wgPageFormsEDSettings;

		// make sure to add 'data-value-initial' attribute even if it's empty (create page case)
		// this is required by SemanticFormsSelect to check, if PF does support 'data-value-initial'
		...
		$inputAttrs = array(
			...
			'value' => $cur_value,
			'data-value-initial' => $initial_value,
			'size' => $size,
