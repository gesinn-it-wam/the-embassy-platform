...
END;
					...
					if ( $tif->getTemplateName() === '' || $field_name == '#freetext#' ) {
                        ...
                        // save the initial value before any mapping happens (for later use)
						$initial_value = $cur_value;
						...
						$new_text = $this->formFieldHTML( $form_field, $cur_value, $initial_value );
	 ...
	 *
	 * Both, $cur_value and $cur_value_initial could be single or multi value
	 * In case of multi value, we can assume that all items in both arrays have the same order
	 * @param PFFormField $form_field
	 * @param string $cur_value
	 * @return string
	 */
	function formFieldHTML( $form_field, $cur_value, $initial_value ) {
		global $wgPageFormsFieldNum;
		...
		if ( $class_name !== null ) {
			switch ( $class_name ) {
				case "PFComboBoxInput":
				case "PFTokensInput":
					$form_input = new $class_name( $wgPageFormsFieldNum, $cur_value, $initial_value, $form_field->getInputName(), $form_field->isDisabled(), $other_args );
					break;
				default:
